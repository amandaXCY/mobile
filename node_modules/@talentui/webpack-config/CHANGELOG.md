### 4.1.7
* 去掉terser plugin的extractComment，避免在生成的output中有license.txt文件

### 4.1.6
* 升级clean webpack plugin到最新版本 3.x

### 4.1.5
* 添加jsSourceMap参数，用来控制是否启用jsSourceMap, 默认为true

### 4.1.4
* 添加outputHtml参数，用来控制是否启用html-webpack-plugin, 默认为true

### 4.1.2-beta.27
* 添加css sourcemap的支持，添加参数cssSourceMap

### 4.1.2-beta.25
* 环境变量支持publicPath
* 优化extractStyles的取值逻辑

### 4.1.2-beta.23
* 开发环境使用cheap-source-map生产环境使用source-map
* 非生产构建时打开disableHostCheck

### 4.1.2-beta.22
* 添加cleanOutput参数，用来控制构建时是否清理上次构建的文件

### 4.1.2-beta.19
* 解决非生产环境构建下的重名文件的覆盖问题

### 4.1.2-beta.18
* 添加babelrc参数，用来控制是否使用自定义babel配置

### 4.1.2-beta.15
* 添加tmplData参数，用来向模板传递自定义数据

### 4.1.2-beta.14

* 开启mini-css-extract-plugin的long time cache

### 4.1.2-beta.13
* 去掉html webpack plugin当中chunk sort mode参数

### 4.1.1
* 添加css modules的支持，添加参加cssModules
